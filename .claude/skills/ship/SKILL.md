...
Use the PR body templates in `reference.md` § PR Body Templates in this
skill directory (task-linked and standalone variants, plus the conflict
resolution section when the rebase had conflicts).

#### Step 7a.1: Embed Technical Reviews in PR Body

If claimed tasks were found in Step 1a, embed their technical review
files in the PR body as collapsible sections — follow `reference.md`
§ Embedding Technical Reviews. If no review file exists for a task,
skip silently.
...
Present progress using the block format in `reference.md` § Output
Format (task header, CI parity, commit, rebase, push, PR, merge, sync
sections). Read it before printing the report.

## Error Handling

The error → resolution table lives in `reference.md` § Error Handling.
Read it when a step fails or an unexpected state appears.
